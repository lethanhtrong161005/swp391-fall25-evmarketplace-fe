...
function getMemberActions(order) {
  const remain = Math.max(
    0,
    Number(order.amount || 0) - Number(order.paidAmount || 0)
  );
  const actions = [
    { key: "detail", label: "Xem chi tiết", icon: <FileTextOutlined /> },
  ];
  ...
  if (
    ["CONTRACT_SIGNED", "COMPLETED"].includes(order.status) ||
    order.contractAvailable
  ) {
    actions.push({
      key: "contract",
      label: "Tải hợp đồng",
...
export default function OrderTable({
  ...
  const handleActiveContract = async (record) => {
    try {
      const payload = {
        contractId: record.contractId,
        buyerId: currentUserId,
      };
      await activeContract(payload);
      message.success("Kích hoạt hợp đồng thành công!");
      reloadOrders?.();
    } catch (err) {
      message.error(err?.message || "Kích hoạt hợp đồng thất bại!");
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (title, r) => (
        <Space size={4} direction="vertical">
          <span className={s.strong} title={title}>
            {title}
          </span>
    ...
    },
    {
      ...
      width: 200,
      render: (_, r) => {
        // Nếu hợp đồng đã upload nhưng chưa active
        if (r.contractStatus === "UPLOADED" && r.contractId) {
          return (
            <Button
              type="primary"
              size="small"
              icon={<ThunderboltOutlined />}
              onClick={() => handleActiveContract(r)}
            >
              Kích hoạt hợp đồng
            </Button>
          );
