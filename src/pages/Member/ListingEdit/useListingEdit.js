...
export function useListingEdit({ userId = null, listingId = null } = {}) {
  ...
  const safeSetFetching = (v) => {
    if (mountedRef.current) setFetching(v);
  };
  ...
  const mapListingToForm = (data) => {
    ...
    return {
      ...
      address: {
        line: l.address,
        province: l.province
          ? { value: l.province, label: l.province }
          : undefined,
        district: l.district
          ? { value: l.district, label: l.district }
          : undefined,
  ...
  const loadListingForEdit = useCallback(
    async (id) => {
      if (!id) return;
      try {
        ...
        if (res?.data) {
          ...
          const catOpt = (tax?.categoryOptions || []).find(
            (o) => o.value === listingData.listing.categoryId
          );
        ...
        msg.error(
          "Lỗi khi tải dữ liệu bài đăng: " + (err?.message || "Unknown error")
        );
      } finally {
  ...
  const isBattery =
    (forcedItemType ??
      (selectedCategory?.code === "BATTERY" ? "BATTERY" : "VEHICLE")) ===
    "BATTERY";
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (res?.success !== false) {
        msg.success("Cập nhật tin thành công!");
        ...
        msg.error(res?.message || "Cập nhật tin thất bại.");
      }
    } catch (e) {
      if (e?.errorFields)
