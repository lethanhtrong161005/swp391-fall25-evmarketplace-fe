...
const useManagerListing = () => {
    ...
    const refetchPage = useCallback(
        async (p = page, s = pageSize) => {
            ...
            const res = await fetchMyListings({ page: p, size: s, status: activeTab, q: query || undefined });
            setServerPage(res);
        },
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                const res = await fetchMyListings({ page, size: pageSize, status: activeTab, q: query || undefined });
    ...
    const counts = useMemo(() => {
        ...
        for (const k of Object.keys(serverCounts || {})) base[k] = serverCounts[k] ?? 0;
    ...
    const _afterMutate = useCallback(async () => {
        const currCount = Array.isArray(serverPage.items) ? serverPage.items.length : 1;
    ...
    // Ẩn (ACTIVE → HIDDEN)
    const onHide = useCallback(async (row) => {
    ...
    const onSoftDelete = useCallback(async (row) => {
        ...
        try {
            const res = await changeStatusListing({ id: row.id, status: "SOFT_DELETED" });
            if (res.status === 200 && res.data?.success) {
