    ...
    <div style={{ padding: "24px", background: token.colorBgLayout }}>
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={12}>
            ...
            >
              {txState.loading ? (
                ...
                <Alert
                  type="error"
                  message="Lỗi tải Transactions"
                  description={
                    <Button icon={<ReloadOutlined />} onClick={retryTx}>
                      Retry
                    </Button>
                  }
                />
                ...
                <Space direction="vertical" style={{ width: "100%" }} size={16}>
                  <Row gutter={16}>
                    <Col span={6}>
                      <Statistic title="Tổng giao dịch" value={txVM.total} />
                    </Col>
                    <Col span={6}>
                      <Statistic title="Thành công" value={txVM.successful} />
                    </Col>
                    <Col span={6}>
                      <Statistic
                        ...
                        valueRender={() => (
                          <Tooltip
                            title={txVM.total === 0 ? "No data" : undefined}
                          >
                            {formatPercent(txVM.successRate)}
                          </Tooltip>
                        )}
                      />
                    </Col>
                    <Col span={6}>
                      <Statistic title="Failed/Huỷ" value={txVM.failed} />
                    </Col>
                  </Row>
                  <Area
                    height={240}
          ...
          <Col xs={24} lg={12}>
            <Card
              title="Revenue"
              extra={
                <Button icon={<DownloadOutlined />} onClick={exportRev}>
                  Export CSV
                </Button>
              }
            >
              {revState.loading ? (
                ...
                <Alert
                  type="error"
                  message="Lỗi tải Revenue"
                  description={
                    <Button icon={<ReloadOutlined />} onClick={retryRev}>
                      Retry
                    </Button>
                  }
                />
                  ...
                  <Row gutter={16}>
                    <Col span={6}>
                      <Statistic
                        ...
                        value={formatCurrency(revVM.total, revVM.currency)}
                      />
                    </Col>
                    <Col span={6}>
                      <Statistic
                        title="TB/ngày"
                        value={formatCurrency(revVM.avgPerDay, revVM.currency)}
                      />
                    </Col>
                    <Col span={6}>
                      <Statistic
                        title="TB/user giao dịch"
                        value={formatCurrency(revVM.avgPerUser, revVM.currency)}
                      />
                    </Col>
                    <Col span={6}>
                      <Statistic
                        title="TB/giao dịch"
                        value={formatCurrency(revVM.avgPerTx, revVM.currency)}
                      />
                    </Col>
                  </Row>
                  <Row gutter={16}>
                    <Col span={12}>
                      <Statistic
                        title="Consignment / Total"
                        valueRender={() => (
                          <Tooltip
                            title={revVM.total === 0 ? "No data" : undefined}
                          >
                            {formatPercent(revVM.consignmentRatio)}
                          </Tooltip>
                        )}
                      />
                    </Col>
        ...
        <Card
          title="Market"
          extra={
            <Button icon={<DownloadOutlined />} onClick={exportMarket}>
              Export CSV
            </Button>
          }
        >
          {marketState.loading ? (
            ...
            <Alert
              type="error"
              message="Lỗi tải Market"
              description={
                <Button icon={<ReloadOutlined />} onClick={retryMarket}>
                  Retry
                </Button>
              }
            />
            ...
            <Space direction="vertical" style={{ width: "100%" }} size={16}>
              ...
              <Table
                ...
                dataSource={marketVM.brandsRows}
                ...
                columns={[
                  ...
                  { title: "Count", dataIndex: "count" },
                  {
                    title: "Avg Price",
                    dataIndex: "avgPrice",
                    render: (v) => formatCurrency(v, filters.currency),
                  },
                ]}
              />

              {marketVM.priceBuckets?.length ? (
                <Tabs
                  items={[
                    {
                      key: "hist",
                      label: "Histogram (Price Buckets)",
                      children: (
                        <Column
                          height={240}
                          data={marketVM.priceBuckets.map((p) => ({
                            name: `${p.category}-${p.bucket}`,
                            value: p.value,
                            series: "bucket",
                          }))}
                          xField="name"
