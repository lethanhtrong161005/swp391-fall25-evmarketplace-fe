...
// Nhãn tiếng Việt cho từng status (có thể điều chỉnh theo glossary dự án)
...
// Mảng options {value,label} để đổ vào Select
...
export default function useContractEditModal({ onUpdated } = {}) {
    ...
    const openModal = useCallback((row) => {
        setRecord(row || null);
        setInitialValues({
            status: row?.status ?? "UPLOADED", // default hợp lý trong set này
            effectiveRange: [
    ...
    return {
        open, submitting, record,
        initialValues,
        fileList, setFileList, beforeUpload,
        openModal, closeModal, handleSubmit,
        // Export choices để component dùng
        STATUS_CHOICES,
        STATUS_TEXT,
    };
}
